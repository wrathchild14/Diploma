...
Ko so bile video igre ustvarjene, so ljudje želeli razmišljati o številnih načinih, kako bi jih naredili bolj zanimive - eden od teh načinov je bilo ustvarjanje posebnega zabavnega vedenja v njih. To vedenje je lahko karkoli v igri, ta diplomska naloga se bolj osredotoča na vedenje likov, ki uporabljajo nekakšno umetno inteligenco.

Naš problem umetne inteligence v igrah je od njene filozofije do različnih pristopov, poskušal bom zajeti vse v okviru razuma. Filozofija umetne inteligence iger ni šala, obstajajo načini, ki jo definirajo v video igrah. Prva stvar je, da je umetna inteligenca nepredvidljiva in se vedno uči, zato je v nasprotju s tem, kar bi morale biti video igre. Nasprotniki ali liki v video igrah so narejeni za zabavo in izziv igralca na določeno raven. Če igralec dobi umetno izboljšanega nasprotnika in ga premaga na vseh ravneh, bi se hitro začelo počutiti dolgočasno in nemogoče igrati.

Tako so se skozi leta razvijalci iger trudili in ustvarili napredne tehnike za vse vrste vedenja, da bi bilo igranje video iger zabavno. Ta diplomska naloga bo obravnavala nekaj od številnih tehnik na način, da bomo spoznali, kako delujejo na zabaven in učinkovit način. Prav tako so za nekaj izmed njih izdelani primeri, ki kažejo moč določene tehnike umetne inteligence v igri. Pristopi so sestavljeni iz številnih že videnih, a na koncu je ena eksperimentalna tehnologija, ki mi je bila zelo všeč. Pristopi so:
...
\begin{enumerate}
    ...
    \item Behaviour/Decision Trees
        \begin{itemize}
            \item Sestavljeni so iz vozlišč, ki so vedenja, in pogojev za vsako vozlišče. Kakor obsežni za igro so pokazali, da potrebujejo veliko računske moči.
        \end{itemize}
    \item Scripting
        \begin{itemize}
...
Z nadaljnjim napredovanjem znanosti se bližamo pravi umetni inteligenci, ki je podobna človeški, morda pa bo ta diplomska naloga pokazala da bi si to želeli v igrah če bi jo le temeljito preizkusili in razumeli. Kdo bi želel da se v njegovih igrah zgodijo nepredvidljive stvari?
...
It is in close connection to game AI that there exists two type of games. 
...
\begin{itemize}
    \item \emph{Master scripts} work best in two scenarios. The first one would be if in game already exists some kind of AI, and then the scripting would just combine them. Even in the other scenario when its only the scripting technique, it can produce good results.
...
Fuzzy logic \cite{FuzzyAIGames} is a extension to conventional logic that can handle the concept of partial-truth between the boolean dichotomy of \emph{true} and \emph{false}. It usually has its own components like \emph{fuzzy variables, fuzzy rules and fuzzy interface engines}.
...
Technologies I will use to present the example will be the Unity \cite{UnitySoftware} editor and engine again. This implementation is based on Craig Reynold's program \cite{FlocksReynolds} to simulate flocking behaviour. So, the example works in a way that each boid steers itself through separation, cohesion, alignment in every \texttt{Update()} call. So, that means its a pretty expensive, but for a demonstration, it will do just fine. Most of the time it a has a time complexity of $ \mathcal{O}(n^2) $.
...
After we've checked out some the key game AI that are used in the industry, I would like to present an open-source project the Unity Machine Learning Agents Toolkit or ML-Agents Toolkit \cite{MLAgents}. With a Python \cite{PythonManual} API (Application Programming Interface), it trains intelligent agents through \emph{reinforcement learning, imitation learning, neuroevolution}.
